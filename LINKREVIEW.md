...
| - | Text line detection in handwritten documents | 2008 | Louloudis, Gatos, Pratikakis, Halatsis | https://sci-hub.ru/meta/10.1016/j.patcog.2008.05.011 | - | Алгоритм состоит из трёх шагов (один из них - преобразование Хафа). Оценка вычисляется на основе перекрытия наборов пикселей, помеченных как строки текста, и эталонных данных (MatchScore). Подход Хафа показывает лучшую производительность, чем два других метода (метод нечеткой длины серий, а также метод профиля проекции), с точки зрения коэффициента обнаружения. |